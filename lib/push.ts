...
export async function sendPush(subscription: StoredPushSubscription, payload: { title: string; body: string; url?: string }) {
  return configureWebPush().sendNotification(subscription, JSON.stringify({ icon: "/brand/logo/1x/logo-light.png", badge: "/brand/logo/1x/logo-light.png", url: payload.url ?? "/#tasks", ...payload }), { timeout: outboundRequestTimeoutMs });
}
