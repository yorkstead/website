...
const operators = [
  "Small manufacturers",
  "CNC, sign, and fabrication shops",
  "Contractors and field-service businesses",
  "Small companies beyond spreadsheets, paper, and disconnected software",
];
...
const operatingExperience = ["Production management", "CNC fabrication", "Inventory", "Shipping", "Operations", "Software development"];
...
export default function Home() {
  return <main className="min-h-screen overflow-hidden"><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(organizationStructuredData).replace(/</g, "\\u003c") }} /><div className="pointer-events-none fixed inset-0 bg-[radial-gradient(circle_at_75%_0%,color-mix(in_oklab,var(--primary)_15%,transparent),transparent_38%)]" />
    ...
    <FounderIntroduction />
    <EngagementPricing />
    <section id="work" className="relative mx-auto max-w-7xl px-5 py-24 sm:px-8"><div className="mb-12"><div className="flex flex-col justify-between gap-5 md:flex-row md:items-end"><div className="max-w-2xl"><div className="font-mono text-[10px] uppercase tracking-[0.24em] text-primary">Selected work</div><h2 className="mt-3 text-3xl font-semibold tracking-tight sm:text-4xl">Systems with an operating point of view.</h2><p className="mt-4 text-sm leading-6 text-muted-foreground">Five live systems and two working prototypes, spanning production release control, internal operations, online commerce, inventory visibility, hospitality point of sale, employee scan workflows, and authenticated production analytics. Each profile states what exists today, what remains intended, and where evidence is still limited.</p></div><a href="https://github.com/rivetworks" target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 text-sm text-muted-foreground transition hover:text-foreground"><Github className="size-4" />View GitHub <MoveUpRight className="size-3.5" /></a></div><ProjectStatusLegend /></div><div className="space-y-4">{caseStudies.map((study) => <CaseStudyCard key={study.slug} study={study} />)}</div></section>
    <section id="contact" className="relative border-t border-border bg-card/35"><div className="mx-auto grid max-w-7xl gap-10 px-5 py-24 sm:px-8 lg:grid-cols-[.7fr_1.3fr]"><div><div className="font-mono text-[10px] uppercase tracking-[0.24em] text-primary">Project contact</div><h2 className="mt-3 text-3xl font-semibold tracking-tight sm:text-4xl">Have an operating problem worth fixing?</h2><p className="mt-4 max-w-lg text-sm leading-6 text-muted-foreground">Share the messy version. A useful first conversation is about the current workflow, where it breaks, and what better would actually mean.</p><div className="mt-8 space-y-3 text-sm"><div className="flex items-center gap-3"><ShieldCheck className="size-4 text-primary" aria-hidden="true" />Direct and confidential · Brandon York</div><div className="flex items-center gap-3"><Mail className="size-4 text-primary" aria-hidden="true" /><TrackedLink href={brandMailto} event="email_link_click" metadata={{ placement: "homepage-contact" }} className="hover:text-primary">{brand.email}</TrackedLink></div>{publicBusinessDetails.phone && publicBusinessPhoneHref ? <div className="flex items-center gap-3"><Phone className="size-4 text-primary" aria-hidden="true" /><TrackedLink href={publicBusinessPhoneHref} event="phone_link_click" metadata={{ placement: "homepage-contact" }} className="hover:text-primary">{publicBusinessDetails.phone}</TrackedLink></div> : null}<p className="pl-7 font-mono text-[9px] uppercase tracking-[0.16em] text-muted-foreground">{brand.descriptor}</p></div></div><Card className="bg-background/70"><CardContent className="p-6 sm:p-8"><Suspense fallback={<div className="h-96 animate-pulse rounded-lg bg-muted" aria-label="Loading contact form" />}><ContactForm /></Suspense></CardContent></Card></div></section>
    <SiteFooter><nav aria-label="Service pages" className="flex flex-wrap gap-x-5 gap-y-2 lg:justify-end">{publicServices.map((service) => <Link key={service.slug} href={`/services/${service.slug}`} className="transition hover:text-foreground">{service.name}</Link>)}</nav></SiteFooter>
  </main>;
}
